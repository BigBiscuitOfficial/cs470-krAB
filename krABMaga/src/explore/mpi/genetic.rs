...
macro_rules! explore_ga_distributed_mpi {
    (
        ...
        loop {
            ...
            #[cfg(feature = "mpi_verbose_timing")]
            let generation_timer_start = Instant::now();
            #[cfg(feature = "mpi_verbose_timing")]
            let mut compute_time_local = 0.0f64;
            #[cfg(feature = "mpi_verbose_timing")]
            let mut communication_time_local = 0.0f64;
            #[cfg(feature = "mpi_verbose_timing")]
            let mut root_ops_time_local = 0.0f64;

            let mut samples_count: Vec<Count> = Vec::new();

            #[cfg(feature = "mpi_verbose_timing")]
            let comm_distribution_timer_start = Instant::now();
            ...
            if world.rank() == root_rank {
                ...
                for i in 0..num_procs {
                    ...
                    // send the arrays only to workers; rank 0 keeps its chunk locally
                    if i == 0 {
                        send_index += sub_population_size;
                    } else {
                        world.process_at_rank(i as i32).send(&sub_population_size);
                        for _p in 0..sub_population_size {
                            world
                                .process_at_rank(i as i32)
                                .send(&population_params[send_index].clone().as_bytes()[..]);
                            send_index += 1;
                        }
                    }
            ...
            #[cfg(feature = "mpi_verbose_timing")]
            {
                communication_time_local += comm_distribution_timer_start.elapsed().as_secs_f64();
            }
            ...
            #[cfg(feature = "mpi_verbose_timing")]
            let compute_timer_start = Instant::now();
            ...
            #[cfg(not(feature = "parallel"))]
            {
                for individual_params in my_population.iter_mut() {
                    ...
                    let mut computed_ind: Vec<($state, Schedule)> = Vec::with_capacity(reps as usize);
                    ...
                    let result = BufferGA::new(generation, local_index, fitness);
            ...
            #[cfg(feature = "parallel")]
            {
                // 1. Calculate the slice limit so the master respects my_pop_size safely
                let limit = if my_rank == 0 {
                    std::cmp::min(my_population.len(), my_pop_size - master_counter)
                } else {
                    my_population.len()
                };

                // 2. PARALLEL MAP PHASE: Run the simulations across CPU cores
                let fitness_results: Vec<(f32, String)> = my_population[..limit]
                    .par_iter()
                    .map(|individual_params| {
                        let mut computed_ind: Vec<($state, Schedule)> = Vec::with_capacity(reps as usize);
                        ...
                        for r in 0..(reps as usize) {
                            ...
                            for _ in 0..($step as usize) {
                                let ind_state = individual.as_state_mut();
                                schedule.step(ind_state);
                                if ind_state.end_condition(&mut schedule) {
                                    break;
                                }
                            }
                            computed_ind.push((individual, schedule));
                        ...
                        let fitness_val = $fitness(&mut computed_ind);
                        (fitness_val, individual_params.clone())
                    })
                    .collect();

                // 3. SEQUENTIAL REDUCE PHASE: Update tracking variables
                for (fitness, individual_params) in fitness_results {
                    match my_best_fitness {
                        Some(_) => {
                            if $cmp(&fitness, &my_best_fitness.expect("265")) {
                                my_best_fitness = Some(fitness);
                                my_best_index = local_index;
                                my_best_individual = individual_params;
                            }
                        }
                        None => {
                            my_best_fitness = Some(fitness);
            ...
            #[cfg(feature = "mpi_verbose_timing")]
            {
                compute_time_local += compute_timer_start.elapsed().as_secs_f64();
            }

            #[cfg(feature = "mpi_verbose_timing")]
            let comm_best_timer_start = Instant::now();
            ...
            #[cfg(feature = "mpi_verbose_timing")]
            {
                communication_time_local += comm_best_timer_start.elapsed().as_secs_f64();
            }

            #[cfg(feature = "mpi_verbose_timing")]
            let comm_results_timer_start = Instant::now();
            ...
            #[cfg(feature = "mpi_verbose_timing")]
            {
                communication_time_local += comm_results_timer_start.elapsed().as_secs_f64();
            }
            ...
            #[cfg(feature = "mpi_verbose_timing")]
            let comm_flag_timer_start = Instant::now();
            ...
            #[cfg(feature = "mpi_verbose_timing")]
            {
                communication_time_local += comm_flag_timer_start.elapsed().as_secs_f64();
            }
            ...
            #[cfg(feature = "mpi_verbose_timing")]
            let root_ops_timer_start = Instant::now();
            ...
            #[cfg(feature = "mpi_verbose_timing")]
            if world.rank() == root_rank {
                root_ops_time_local += root_ops_timer_start.elapsed().as_secs_f64();
            }

            #[cfg(feature = "mpi_verbose_timing")]
            let generation_time_local = generation_timer_start.elapsed().as_secs_f64();

            #[cfg(feature = "mpi_verbose_timing")]
            if world.rank() == root_rank {
                let mut compute_times = vec![0.0f64; num_procs];
                let mut communication_times = vec![0.0f64; num_procs];
                let mut root_ops_times = vec![0.0f64; num_procs];
                let mut generation_times = vec![0.0f64; num_procs];

                root_process.gather_into_root(&compute_time_local, &mut compute_times[..]);
                root_process
                    .gather_into_root(&communication_time_local, &mut communication_times[..]);
                root_process.gather_into_root(&root_ops_time_local, &mut root_ops_times[..]);
                root_process.gather_into_root(&generation_time_local, &mut generation_times[..]);

                let compute_max = compute_times
                    .iter()
                    .copied()
                    .fold(0.0f64, f64::max);
                let communication_max = communication_times
                    .iter()
                    .copied()
                    .fold(0.0f64, f64::max);
                let root_ops_max = root_ops_times
                    .iter()
                    .copied()
                    .fold(0.0f64, f64::max);
                let generation_max = generation_times
                    .iter()
                    .copied()
                    .fold(0.0f64, f64::max);

                let compute_avg = compute_times.iter().sum::<f64>() / num_procs as f64;
                let communication_avg = communication_times.iter().sum::<f64>() / num_procs as f64;
                let generation_avg = generation_times.iter().sum::<f64>() / num_procs as f64;
                let overhead_ratio = if generation_max > 0.0 {
                    (communication_max + root_ops_max) / generation_max
                } else {
                    0.0
                };

                println!(
                    "[MPI_TIMING] gen={} total_max={:.6}s total_avg={:.6}s compute_max={:.6}s compute_avg={:.6}s comm_max={:.6}s comm_avg={:.6}s root_ops={:.6}s overhead_ratio={:.3}",
                    generation,
                    generation_max,
                    generation_avg,
                    compute_max,
                ...
                root_process.gather_into(&compute_time_local);
                root_process.gather_into(&communication_time_local);
                root_process.gather_into(&root_ops_time_local);
                root_process.gather_into(&generation_time_local);
            }
