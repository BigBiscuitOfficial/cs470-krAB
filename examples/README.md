...
**Description:** MPI-based distributed execution for running strategy sweeps across ranks.
...
**Note:** Current MPI mode parallelizes strategy combinations across ranks using the engine-level `krabmaga::explore::mpi::sweep::sweep_mpi` abstraction. Each rank runs assigned strategies locally and the root rank gathers/aggregates final artifacts.
...
**MPI mode:**
- Distributes strategy combinations across ranks (contiguous partitions)
- Uses engine-level `sweep_mpi` to gather per-strategy summaries at rank 0 for final report generation
